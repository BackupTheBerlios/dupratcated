...
import static fr.umlv.symphonie.view.SymphonieConstants.COMMENT;
import static fr.umlv.symphonie.view.SymphonieConstants.COURSE;
import static fr.umlv.symphonie.view.SymphonieConstants.JURY_CHART_STUDENTS_AVERAGES;
import static fr.umlv.symphonie.view.SymphonieConstants.JURY_HEADER;
 ...
 * 
 */
public class JuryModel extends AbstractSymphonieTableModel {

  /**
   * The list of columns in the model, except the first and the last ones. It
   * can contain <code>Formulas</code> s and <code>Course</code> s
   */
  ...
  /**
   * The map containing all students, and all marks of each student for each
   * course.
   */
  ...
  public JuryModel(DataManager manager, ComponentBuilder builder) {
    super(manager, builder, CellRendererFactory.getJuryModelCellRenderer());
    update();
  }
  ...
  /**
   * Tells if a cell in the model is editable or not. In the jury model, all you
   * can edit is the comments for each student.
   * 
  ...
  /**
   * Removes a given column from the database and the model. If the given column
   * is not a formula, nothing will be removed.
   * 
   * @param columnIndex
   *          The index in the model of the column to remove.
   */
   ...
   * 
   * @param columnIndex
   *          the index in the model of the column to test.
   ...
   * 
   * @return A <code>MessageFormat</code> containing the header message of the
   *         chart.
   */
  ...
  /**
   * Construct the <code>ChartPanel</code> containing the chart for the jury
   * view.
   * 
   * @param step
   *          The interval of averages for the chart.
  ...
  public ChartPanel getChartPanel(int step) {
    ...
    JFreeChart barChart = ChartFactory.createBarChart3D(builder
        .getValue(JURY_CHART_STUDENTS_AVERAGES), builder
        .getValue(CHART_MARK_INTERVAL), builder.getValue(CHART_STUDENT_NUMBER),
   ...
   * the chart for the jury view.
   * 
   * @param dataTab
   *          The <code>Map</code> array to initialize.
   * @param courseMap
   *          The <code>Map<Integer, Course></code> which contains all data
   ...
   * @param column
   *          The column index where to put the formula.
   */
