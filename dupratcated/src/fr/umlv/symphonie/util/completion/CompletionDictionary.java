...
public class CompletionDictionary implements LookableCollection<String> {
	...
	/**
	 * Looks up the given string in the current dictionary, search is case insensitive
	 * @return <code>null</code> if <code>prefix</code> is null or there's no string that starts with <code>prefix</code>.<br>
	 */
	public String lookup(String prefix) {
		...
		boolean b = false;
		if (rs != null)
			b = rs.toLowerCase().startsWith(prefix.toLowerCase());
		return b ? rs : null;
	}
}
