...
public class XMLExporter implements DataExporter {
  ...
  private void writeDocument(Document document) throws DataExporterException {
    try {
      ...
      throw new DataExporterException("Error during the exportation : \n", e);
  ...
  public void exportStudentView(String documentName, Student s)
    ...
    /**
     * we export all the marks for the student of the student view and all the
     * examens
     */
  ...
  public void exportTeacherView(String documentName, Course c)
    ...
    for (Student s : sortedMap.keySet()) {
      ...
      for (StudentMark sm : map.values()) {
        ...
        /**
         * if it the last student node, we can create the examens nodes since it
         * has to be at the end of the xml
         */
  ...
  public void exportJuryView(String documentName) throws DataExporterException {
    ...
    for (Student s : sortedMap.keySet()) {
      ...
      for (Course c : map.values()) {
        ...
        /**
         * if it the first student node, we can create the courses nodes before
         * the student node since it has to be at the begin of the xml
         */
        ...
        for (StudentMark sm : map2.values()) {
          /**
           * if it the last student node, we can create the examens nodes since
           * it has to be at the end of the xml
           */
          if (idLastStudent == s.getId()) {
            addExamenNode(root, sm);
          }
